...
async def get_column_categories(column: str):
    ...
    return {"categories": [str(cat) for cat in categories][:20]} #return first 20 categories to avoid overwhelming the frontend
...
async def get_table_data(request: TableDataRequest):
    ...
    try:
        ...
        preview_data = filtered_df.head(50).fillna("").to_dict(orient="records") #get first 50 rows after filtering
...
# ----------------------------------------------------
# كود دمج فرونت إند React (أضفه في نهاية الملف تماماً)
# ----------------------------------------------------

# تحديد مسار مجلد dist الذي سينتج عن بناء مشروع React بالـ Vite
frontend_path = os.path.join(os.path.dirname(__file__), "../frontend/dist")

# التحقق من وجود المجلد أولاً (لتجنب المشاكل أثناء التطوير المحلي)
if os.path.exists(frontend_path):
    # تشغيل ملفات الـ React الثابتة (CSS, JS, Images) على الرابط الرئيسي "/"
    app.mount("/", StaticFiles(directory=frontend_path, html=True), name="frontend")
    
    # حل مشكلة الـ Routing في React (إذا قام المستخدم بعمل ريفريش لصفحة فرعية)
